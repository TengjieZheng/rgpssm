...
class IndependentMultitaskGPModel(ApproximateGP):
    ...
    def __init__(self, dim_state, inducing_points, var_task=None, len_scale=None):
        ...
        if var_task is not None:
            self.covar_module.base_kernel.var = torch.tensor(var_task).view(-1, 1)
        if len_scale is not None:
            self.covar_module.base_kernel.lengthscale = torch.tensor(len_scale).view(-1, 1)
