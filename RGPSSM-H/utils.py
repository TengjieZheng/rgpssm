...
def nMSE(x: ndarray, x_pre: ndarray):
    ...
    for i in range(x.shape[-1]):
        nmse_lst.append(nMSE_single(x[:, i], x_pre[:, i]))
    return np.array(nmse_lst)
...
def Mnlp(x: ndarray, x_pre: ndarray, std: ndarray):
    ...
    mnlp_lst = []
    for i in range(x.shape[-1]):
        mnlp_lst.append(Mnlp_single(x[:, i], x_pre[:, i], std[:, i]))
    return np.array(mnlp_lst)

def nMSE_single(x: ndarray, x_pre: ndarray):
    """x (n,)"""
...
def Mnlp_single(x: ndarray, x_pre: ndarray, std: ndarray):
    """Mean Negative Log Probability
    Args:
        x: (n,)
        x_pre: (n,)
...
def get_color(c):
    ...
    elif isinstance(c, str) and c in c_lst:
        return colors[c_lst.index(c)]
    else:
